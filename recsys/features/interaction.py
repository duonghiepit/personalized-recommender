...
def generate_interaction_data(trans_df: pl.DataFrame):
    ...
    for chunk_start in tqdm(
        range(0, len(unique_customers), chunk_size),
        ...
        for customer_id in chunk_customers:
            ...
            customer_articles = {"purchased": set(),
                                 "clicked": set(),
                                 "ignored": set()}
    ...
    final_df = sorted_df.with_columns(
        [
            pl.col("article_id").alias("pre_article_id")
            .shift(1)
            .over("customer_id")
            .fill_null("START")
        ]
